...
congui <- read_sf("D:\\ANDRÉS\\Mis cursos\\mapas_r\\archivos_curso\\leaflet\\conguillio\\conguillio.shp")

araucania <- read_sf("D:\\ANDRÉS\\Mis cursos\\mapas_r\\archivos_curso\\leaflet\\araucania\\araucania.shp") %>% 
  st_transform(crs = 4326)

nahuelbuta <- read_sf("D:\\ANDRÉS\\Mis cursos\\mapas_r\\archivos_curso\\leaflet\\nahuelbuta\\nahuelbuta.shp") %>% 
  st_transform(crs = 4326)

huerquehue <- read_sf("D:\\ANDRÉS\\Mis cursos\\mapas_r\\archivos_curso\\leaflet\\huerquehue\\huerquehue.shp") %>% 
  st_transform(crs = 4326)
